...
public class Note_view extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
    	...
    	DB.setNotesTableId(DB.getNotesTableId()); // add for install first time error
        mDb = new DB(this);
		...
		int style = mDb.getTabStyle(TabsHostFragment.mCurrentTabIndex);
        mDb.doClose();
    ...
    private void populateFields() {
    	mDb.open();
        if (mRowId != null) {
            Cursor note = mDb.queryNote(mRowId);
